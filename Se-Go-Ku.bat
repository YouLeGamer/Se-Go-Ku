...
:welcome
cls
echo Bienvenue dans Se-Go-Ku ! Appuyez sur 6 pour commencer.
echo.
set /p enter="Entree : "
...
echo Se-Go-Ku
echo.
echo Jeu cree par : Yoann Vandomme
echo Idee originale de : Yoann Vandomme
echo Code par : Yoann Vandomme
echo Beta-test par : personne
echo.
echo Se-Go-Ku InfDev 5.0 Dev. Snapshot 34w21a
echo.
echo  You, Inc. (c) Tous droits reserves.
pause
goto :welcome
...
echo Menu Principal
echo.
echo Pour demarrer une bataille, appuyez sur 1.
echo Pour acceder au Segodex, appuyez sur 2.
echo Pour voir les tutoriaux, appuyez sur 3.
echo Pour acceder au menu d'evolution, appuyez sur 4.
echo Pour voir votre inventaire, appuyez sur 5.
echo.
set /p enter="Entree : "
...
if %enter% equ 7 goto :easteregg

::Menu de bataille
:battle_menu
cls
echo Menu de bataille
echo.
echo Ici, vous ferez des batailles contre des monstres afin, si vous avez de la chance, de les attraper, et si vous n'en avez pas, de perdre des segoballs !
echo Je plaisante, bien sûr...
echo.
echo Appuyez sur 1 pour commencer.
echo.
set /p enter="Entree : "
if %enter% equ 1 goto :monsterchoicea
if %enter% neq 1 goto :easteregglalexandre

::Choix au hasard du monstre. Il faut savoir qu'on a une chance sur 31 d'avoir un monstre en particulier : 30 monstres de première génération + 1 cas où il n'y a pas de monstre
:monsterchoicea
...
:notenoughballs
cls
echo Oh non ! Vous n'avez pas assez de segoballs.
pause
goto :main_menu

::Choix du monstre au hasard; un nombre entre 0 et 30, selon la liste
:monsterchoice
...
:nomonster
cls
echo Zut... il n'y a pas de monstre, on dirait...
echo.
pause
goto :main_menu
...
:easteregglalexandre
cls
echo En raison : grand age, installation,
echo Il y a des degats... considerables !
echo Et la ! Archeologique russe ! Grande decouverte !
echo.
echo Voila.
echo Sinon tu connais la legende de l'hippopotame qui perd son oeil dans la riviere ?
echo.
pause
goto :main_menu
...
if %enter% equ 5 goto :origin

::Easter egg
:origin
cls
echo Ca alors ! Un easter egg ! Decidement, tu es fort pour les denicher. Bon, encore un autre.
echo A l'origine, Se-Go-Ku etait... un jeu de cartes.
echo Le but du createur etait de concurrencer Pokemon. Et (sans mentir) le jeu connut un petit succes !
echo Plusieurs personnes, pendant deux semaines, passaient leurs recreations a jouer a Se-Go-Ku. Les avis etaient favorables en grande majorite.
echo Le jeu avait trois types de cartes : les monstres, les batiments et les objets.
echo Les monstres pouvaient etre places en attaque ou en defense, selon l'avis du joueur. On le savait par rapport au sens de la carte. A la verticale signifiait attaque, a l'horizontale defense.
echo Les batiments servaient a proteger les monstres qu'on ne voulait pas utiliser.
echo Les objets donnaient des capacites speciales aux monstres qui les equipaient.
echo Je vous raconte tout ca, mais vous vous en fichez un peu, pas vrai ? Mais voulez-vous savoir pourquoi le jeu s'est essoufle d'un seul coup ?
echo Alors que le createur voulait rajouter de nouvelles cartes, pour diversifier le jeu, il decouvrit Zeld... heu... connut une baisse de motivation et arreta de creer des cartes. Cela fait deux ans que personne n'a joue au jeu.
echo.
echo Ca vous a interesse ? Continuez donc a chercher des Easter Eggs, il y en a quelques-uns dans le jeu...
pause
goto :main_menu
...
echo Inventaire
echo.
echo Bienvenue dans l'Inventaire ! Ici, vous pourrez voir combien de segoballs, combien de pierres mystiques etc... vous possedez.
echo.
...
echo Nombre de Pierres mystiques de Terre : %terramysticstones%
echo Nombre de Perles mystiques de Terre : %terramysticpearls%
echo.
echo Appuyez sur 9 pour continuer...
echo.
set /p enter="Entree : "
if %enter% equ 9 goto :main_menu

::Easter egg du menu (il est loin lol)
:easteregg
cls
echo Petit malin, va ! Il n'y avait pas ecrit d'appuyer sur 7 dans le menu !
echo.
echo Mais bon, si tu es la, c'est que tu n'est pas bete; et j'aime bien les gens pas betes. 
echo Savais-tu que le createur de ce logiciel, Yoann, etait turbo-con ?
echo Un jour, alors qu'il travaillait sur le Segodex, il supprima malencontreusement la sauvegarde principale du jeu (celle-ci), perdant TOUT ce qu'il avait code jusqu'ici (en gros il devait tout recommencer de zero).
echo Se frappant lui-meme car cela faisait 3 heures qu'il travaillait sur le logiciel, sans compter ce qu'il avait fait avant, il chercha de toutes les manieres possibles comment restaurer le jeu.
echo Et c'est ainsi qu'il le vut !
echo Comme il travaillait sur le bloc-notes, et comme il n'avait pas ferme le code supprime, il le copia, le remit dans un nouveau batch, de sorte à ce que tout soit sauve. C'est ainsi que naquit la deuxieme copie du jeu, contenue dans le dossier backup.
echo.
echo Voila.
echo.
echo Y'a plus d'easter egg pour cette partie, ca ne sert a rien de chercher...
pause
goto :main_menu

::Segodex. Très grosse partie.
:segodex
cls
echo Segodex
echo.
...
:: Finalement, ce programme aura fait 3047 lignes, et 85 Ko.
pause>nul  